...
export function AgentCard({ agente }: AgentCardProps) {
  ...
  return (
    <motion.div
      data-cursor="perfil"
      className="relative flex-shrink-0 w-[220px] rounded-[18px] border bg-[#0d1a10] p-6 flex flex-col items-center text-center overflow-hidden"
      onHoverStart={() => setHovering(true)}
      onHoverEnd={() => setHovering(false)}
      animate={{
        y: hovering ? -6 : 0,
        borderColor: hovering ? 'rgba(201,168,76,0.5)' : 'rgba(255,255,255,0.08)',
        boxShadow: hovering
          ? '0 20px 50px rgba(0,0,0,0.4), 0 8px 24px rgba(201,168,76,0.14)'
          : '0 4px 18px rgba(0,0,0,0.2)',
      }}
      transition={{ duration: 0.4, ease: EASE }}
    >
      {/* Glow dorado sutil de fondo al hover */}
      <motion.div
      ...
      <div className="text-white font-semibold text-[15px] leading-tight z-10">
        {agente.nombre} {agente.apellido}
      </div>
      <div className="text-white/40 text-[11.5px] mt-1 z-10">Asesor/a Inmobiliaria</div>

      <div className="w-full border-t border-white/06 my-4 z-10" />

      <div className="text-white/55 text-[12px] mb-4 z-10">
      ...
      <a
        ...
        className="w-full flex items-center justify-center gap-2 bg-gold/10 border border-gold/25 text-gold text-[12px] font-semibold py-2.5 rounded-lg hover:bg-gold/20 hover:border-gold/45 transition-all duration-300 z-10"
      >
