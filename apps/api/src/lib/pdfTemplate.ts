...
export function buildPdfHtml(data: PdfData): string {
  ...
  const specs = [
    ...
    data.banos != null       ? `<div class="spec"><span class="sv">${data.banos}</span><span class="sk">Banos</span></div>` : '',
...
.agent-avatar { width:72px; height:72px; border-radius:50%; object-fit:cover; border:2px solid #C9A84C; flex-shrink:0; }
.agent-avatar-placeholder { width:72px; height:72px; border-radius:50%; background:linear-gradient(135deg,#0D3B27,#C9A84C); display:flex; align-items:center; justify-content:center; color:#fff; font-size:22px; font-weight:800; flex-shrink:0; }
...
<body>
<div class="header">
  <div><span class="logo-elite">ELITE</span><span class="logo-nuvia">Nuvia</span></div>
...
<div class="content">
  ...
  <div class="sec-label">Descripcion</div>
  <div class="desc">${data.descripcion}</div>
  ${data.fotosUrls.length > 1 ? `<div class="sec-label">Galeria</div><div class="foto-grid">${fotoGrid}</div>` : ''}
  <div class="divider"></div>
  <div class="agent-row">
    ${data.agente.fotoUrl
      ? `<img class="agent-avatar" src="${data.agente.fotoUrl}" alt="${data.agente.nombre}" />`
    ...
    <div class="qr-box">
      ...
      <div class="qr-label">Escanear para contactar</div>
    </div>
  </div>
</div>
<div class="footer">
